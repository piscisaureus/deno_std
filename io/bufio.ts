...
export class BufReader implements Reader {
   ...
   * bytes stop being valid at the next read call. If an error or EOF is
   * encountered before `n` bytes are available, `peek()` throws an error with
   * the `partial` property set to a slice of the buffer that contains the bytes
   * that were available before the error occurred.
   */
