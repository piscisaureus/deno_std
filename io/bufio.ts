...
export class BufReader implements Reader {
  ...
  async readLine(): Promise<ReadLineResult | EOF> {
    ...
    try {
      ...
      // just return whatever is available and set the `isPrefix` flag.
   ...
   * buffer fills without finding a delim, it throws an Error with a `partial`
   * property that contains the entire buffer.
   *
   * If `readSlice()` encounters an EOF and there are any bytes left in the
   * buffer, the rest of the buffer is returned. In other words, EOF is always
   * treated as a delimiter. Once the buffer is empty, it returns `EOF`.
   *
   ...
   * being valid at the next read call. If an error or EOF is encountered before
   * `n` bytes are available, peek() throws an error with the `partial` property
